   ...
   @if ($post->image && $post->created_at < '2018-09-01 00:00:00')
	   <div class="row">        
			<div class = "col-lg-12" id="main_image">
				@if(file_exists(public_path().'/img/secondary-posts/full/'.$post->image))
					<img  src="/img/secondary-posts/full/{{$post->image}}" alt="{{$post->title}}" class="img-responsive">	
				@else
					<img  src="/img/secondary-posts/{{$post->image}}" alt="{{$post->title}}" class="img-responsive">
				@endif			
			</div>        
		</div>
   @endif
